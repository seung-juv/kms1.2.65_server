...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var text = "더 많은 진리를 원하는 자만이 전직을 할 수 있는 자격이 있다.\r\n\r\n#b";
        if (cm.getJob() >= 200 && cm.getJob() < 300 && cm.getPlayerStat("LVL") >= 50) {
            var r = 0;
        ...
        if (mySel == 1) {
            ...
            if (val == null || val.equals("0")) {
                if (cm.getJob() >= 200 && cm.getJob() < 300 && cm.getJob() % 10 == 0 && cm.getPlayerStat("LVL") >= 70) {
                    cm.sendYesNo("흠. 무슨 일로 날 찾아온겐가? 흠... 3차 전직을 통해 좀 더 강한 마법사가 되고 싶다는 건가. 물론 내 힘으로 자네를 좀 더 강하게 만들어 줄 수는 있지만 그 전에 자네가 얼마나 열심히 수련했는지에 대한 검증이 필요하다네. 지금껏 강해지고 싶다는 마음에 날 찾아온 젊은이들은 많았지만 실제로 자신의 강함을 증명한 녀석은 별로 없었지... 어떤가? 쉽지는 않겠지만 자네를 시험해 봐도 괜찮겠는가?");
                }
            } else if (val.startsWith("job3_trial1")) {
                if (cm.haveItem(4031057, 1)) {
                    cm.sendNext("오오.. #b하인즈#k가 준 임무를 무사히 완수한 모양이로군. 역시 자네라면 해낼 수 있을거라고 생각했네. 하지만 두번째 시험이 남아있다는 걸 잊지는 않았겠지? 두번째 시험에 앞서 우선은 목걸이를 받도록 하겠네.");
                } else {
                    cm.sendOk("자네에게 1차, 2차 전직을 시켜주었던 엘리니아의 #b하인즈#k를 기억하고 있겠지? 그를 찾아가면 그가 자네에게 한가지 임무를 줄 것일세. 그 임무를 무사히 수행하고 하인즈로부터 #b강인함의 목걸이#k를 받아오게나.");
                    cm.dispose();
                }
        ...
        if (val == null || val.equals("0")) {
            if (cm.getJob() >= 200 && cm.getJob() < 300 && cm.getJob() % 10 == 0 && cm.getPlayerStat("LVL") >= 70) {
                if (selection == 1) {
                    cm.sendOk("좋네! 자네가 증명해야 할 것은 자네가 가진 힘과 자네가 가진 지혜, 이 두 가지라네. 우선은 힘에 대한 시험부터 설명을 해 주겠네. 자네에게 1차, 2차 전직을 시켜주었던 엘리니아의 #b하인즈#k를 기억하고 있겠지? 그를 찾아가면 그가 자네에게 한가지 임무를 줄 것일세. 그 임무를 무사히 수행하고 하인즈로부터 #b강인함의 목걸이#k를 받아오게나.");
