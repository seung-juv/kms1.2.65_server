 ...
 * ScriptName : ludi026
 * NPCNameFunc : 위습
 * Location : 220000400 (루디브리엄 - 에오스탑 입구)
 * 
 * @author T-Sun
 *
 */
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendSimple("저는 펫을 되살려 주거나, 기존 펫의 능력치를 새로운 펫으로 옮겨주는 일을 맡고 있는 정령, 위습이에요.#b\r\n\r\n#L2# 죽은 펫을 다시 살리고 싶어요.#l");
        } else if (status == 1) {
            if (selection == 2) { //revive	
                val = cm.getQuestStatus(3240);
                ...
                if (theitems.length <= 0) {
                    ...
                    if (val == 0 || val == 2) {
                        ...
                        if (cm.haveItem(4031034, 1) && cm.haveItem(5180000, 1)) {
                            ...
                            cm.sendOk("준비해 오셔야 할 재료는 #b생명의 물#k 과 #r생명의 주문서#k 입니다. 생명의 물은.. 구하기 힘들지만.. 어떤 상점에서 현재 판매하고 있다고 하더군요.  생명의 주문서는 루디브리엄 마을의 집을 돌아다녀 보시면 얻을 수 있을지도 모르겠군요.");
                            cm.dispose();
                        }
                    }
                }
