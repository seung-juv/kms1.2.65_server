 ...
 * NPCNameFunc : 티티앙 - 매표소 안내원
 * Location : 220000110 (루디브리엄 - 정거장<오르비스행>)
 * 
 * @author T-Sun
 *
 */
...
function start() {
    status = -1;
    boat = cm.getEventManager("Trains");
    action(1, 0, 0);
}
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var str = "배는 매 시간 정각 기준으로 10분 마다 출발하고 있으며, 출발 5분 전부터 표를 받고 있답니다.\r\n";
        ...
        if(boat == null) {
            ...
            cm.sendOk("아직 배가 출항 준비중에 있습니다. 배는 매 시간 정각 기준 5, 15, 25, 35, 45, 55분에 출항을 준비하며, 최장 30분 이내에 출항 준비가 완료됩니다.");
            cm.dispose();
            ...
            cm.sendOk("이미 배가 오르비스로 출발했답니다. 배는 매 시간 기준으로 10분 마다 출발하니 잠시만 기다려 보세요.");
            cm.dispose();
        }
    } else if (status == 2) {
