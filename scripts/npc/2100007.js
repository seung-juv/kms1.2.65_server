...
/*
 * NPCID : 2100007
 * ScriptName : skin_ariant1
 * NPCNameFunc : 라일라 - 피부관리사
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
	cm.sendNext("아름다운 피부로 가꾸시고 싶으신가요? 제가 도와드리도록 하지요. 서비스를 이용하려면 #b#t5153007##k 아이템을 가져오셔야 합니다.");
    } else if (status == 1) {
	cm.sendStyle("원하시는 피부를 골라보세요~", skin);
    } else if (status == 2){
	if (cm.setAvatar(5153007, skin[selection]) == 1) {
