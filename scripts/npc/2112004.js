...
function action(mode, type, selection) {
    ...
    switch (cm.getPlayer().getMapId()) {
        ...
        case 926100401:
            ...
            break;
        case 926110401:
            cm.sendOk("구해주셔서 진심으로 감사드립니다.");
            break;
        case 926110600:
            cm.sendOk("저를 구해주신 모험가님. 진심으로 감사드립니다.")
            break;
        case 926100600:
            status++;
            if (status == 0) {
                cm.sendNext("이렇게 유레테도 마음을 고쳐먹었고, 제뉴미스트와 알카드노의 전쟁도 조금은 수그러 들 수 있을까요? 비록 저희 사랑이 이뤄지기까지 아직도 험난한 고난이 기다리고 있지만, 결코 포기하지 않고 줄리엣을 지키겠습니다.");
                return;
            } else if (status == 1) {
