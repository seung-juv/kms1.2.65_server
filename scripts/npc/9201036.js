...
function action(mode, type, selection) {
    ...
    if (cm.getPlayer().getMapId() == 680000200) {
		...
		cm.getPlayer().setWeddingGive(selection);
            if (selection == 0) {
                cm.getClient().sendPacket(Packages.tools.MaplePacketCreator.showWeddingWishGiveDialog(entry.getGroomWishList()));
            } else {
		cm.getClient().sendPacket(Packages.tools.MaplePacketCreator.showWeddingWishGiveDialog(entry.getBrideWishList()));
            }
        ...
        if (entry != null) {
            if (entry.getStatus() >= 2 && entry.getWeddingStatus() >= 8) {
                ...
                if (gifts.isEmpty()) {
                    ...
                    cm.getClient().sendPacket(Packages.tools.MaplePacketCreator.showWeddingWishRecvDialog(gifts));
                }
                cm.dispose();
            } else {
