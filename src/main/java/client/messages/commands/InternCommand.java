...
public class InternCommand {
    ...
    public static class TempBan extends CommandExecute {
        ...
        private final String[] types = {"핵 사용", "매크로 사용", "광고", "욕설 / 비난 / 비방", "도배", "GM 괴롭힘 / 욕", "공개 욕설/비난/비방", "현금거래", "임시 정지 처분", "사칭", "관리자 사칭", "불법 / 비인가 프로그램 사용 (감지)", "계정 도용"};
    ...
    public static class UnHellBan extends 벤해제 {
    ...
    public static class 검색 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length == 1) {
                ...
                MapleData data = null;
                MapleDataProvider dataProvider = MapleDataProviderFactory.getDataProvider(new File(ServerProperties.getProperty("server.wz-path") + "/" + "String.wz"));
