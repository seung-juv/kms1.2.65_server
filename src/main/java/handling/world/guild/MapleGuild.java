...
//        gainGP(500, true, mgc.getId());
            ...
            while (itr.hasNext()) {
                ...
                if (mgcc.getId() == mgc.getId()) {
                    ...
                    if (mgc.isOnline()) {
                        ...
                        setOfflineGuildStatus((short) 0, (byte) 5, (byte) 5, mgcc.getId());
                    }
                    break;
                }
            }
    ...
    public final void changeARank(final boolean leader) {
        ...
        for (final MapleGuildCharacter mgc : members) {
            ...
            if (mgc.isOnline()) {
                World.Guild.setGuildAndRank(mgc.getId(), this.id, mgc.getGuildRank(), newRank);
            } else {
                setOfflineGuildStatus(this.id, (byte) mgc.getGuildRank(), (byte) newRank, mgc.getId());
            }
        ...
        for (final MapleGuildCharacter mgc : members) {
            if (cid == mgc.getId()) {
                if (mgc.isOnline()) {
                    ...
                    setOfflineGuildStatus(this.id, (byte) mgc.getGuildRank(), (byte) newRank, cid);
                }
                mgc.setAllianceRank((byte) newRank);
                World.Alliance.sendGuild(allianceid);
                return true;
        ...
        for (final MapleGuildCharacter mgc : members) {
            if (cid == mgc.getId()) {
                if (mgc.isOnline()) {
                    ...
                    setOfflineGuildStatus(this.id, (byte) newRank, (byte) mgc.getAllianceRank(), cid);
                }
...
//                    gainGP((mgc.getLevel() - old_level) * mgc.getLevel(), false, mgc.getId());
    ...
    public final void gainGP(int amount, final boolean broadcast, final int cid) {
        ...
        if (cid > 0 && amount > 0) {
            ...
            if (mg != null) {
                if (mg.isOnline()) {
                    World.Guild.setGuildAndRank(cid, this.id, mg.getGuildRank(), mg.getAllianceRank());
                } else {
                    setOfflineGuildStatus(this.id, (byte) mg.getGuildRank(), (byte) mg.getAllianceRank(), cid);
                }
            }
        }
        gp += amount;
