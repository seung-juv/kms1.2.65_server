...
public class AramiaFireWorks {
    ...
    private static final int[] arrayMob = {9500168, 9500169, 9500170, 9500171, 9500173,
        ...
        9400569};
    private static final int[] arrayX = {900, 1000, 1050, 1100, 1200, 1250, 1300, 1350,
        1400, 1500, 1600, 1650, 1700, 1800, 1950};
    private static final int[] arrayY = {450, 450, 450, 450, 450, 450, 450, 450, 450, 450,
        450, 450, 450, 450, 450};
    ...
    private static final int[] eq = {1002001, 1002012, 1002019, 1002033, 1002041, 1002042, 1002054, 1002060, 1002061, 1002062,
        1002063, 1002080, 1002081, 1002082, 1002083, 1002089, 1002090, 1002132, 1002133, 1002134,
        1002452, 1002453, 1002454,
        1002455, 1032000, 1032001, 1032002, 1032003, 1032009, 1032016, 1032017, 1032019,
        1032020, 1032021, 1032022, 1040013, 1041012, 1060004, 1060005,
        ...
        1102027, 1102028, 1102029, 1102030, 1102031, 1102032, 1102033, 1102034, 1102035,
        1302007, 1302016, 1312000, 1322003, 1322007, 1322008, 1322009,
        1322027, 1332020, 1372005, 1372006, 1432008, 1002002, 1002003, 1002004, 1002005,
        ...
        1372001, 1372002, 1372003, 1372004, 1372009, 1372012, 1372014, 1372015, 1372016,
        ...
        1302006, 1332006, 1332008, 1332009, 1332010, 1332016, 1332019, 1332022,
        1332026, 1402001, 1412001, 1422000, 1422004, 1432000, 1432001, 1442000, 1442004,
    ...
    private static final int[] sc = {2040008, 2040009, 2040010, 2040011, 2040012, 2040013, 2040028, 2040030, 2040304, 2040305,
        2040306, 2040307, 2040320, 2040322, 2040325, 2040327, 2040404, 2040405, 2040406, 2040407,
        2040408, 2040409, 2040424, 2040426, 2040508, 2040509, 2040510, 2040511, 2040518, 2040519,
        2040520, 2040521, 2040531, 2040533, 2040604, 2040605, 2040606, 2040607, 2040608, 2040609,
        2040624, 2040626, 2040712, 2040713, 2040714, 2040715, 2040716, 2040717, 2040808, 2040809,
        2040810, 2040811, 2040812, 2040813, 2040904, 2040905, 2040906, 2040907, 2040908, 2040909,
        2040930, 2040932, 2041026, 2041027, 2041028, 2041029, 2041030, 2041031, 2041032, 2041033,
        2041034, 2041035, 2041036, 2041037, 2041038, 2041039, 2041040, 2041041, 2043004, 2043005,
        2043016, 2043018, 2043104, 2043105, 2043111, 2043113, 2043204, 2043205, 2043211, 2043213,
        2043304, 2043305, 2043704, 2043705, 2043804, 2043805, 2044004, 2044005, 2044011, 2044013,
        2044104, 2044105, 2044111, 2044113, 2044204, 2044205, 2044211, 2044213, 2044304, 2044305,
        2044311, 2044313, 2044404, 2044405, 2044411, 2044413, 2044504, 2044505, 2044604, 2044605,
        2044704, 2044705, 2044803, 2044804, 2044806, 2044808, 2044903, 2044904, 2040001, 2040002, 2040004, 2040005, 2040025, 2040026, 2040029, 2040031, 2040301, 2040302,
    ...
    private final void broadcastEvent(final MapleCharacter c) {
        ...
        EventTimer.getInstance().schedule(new Runnable() {
            ...
            public final void run() {
                startEvent(c.getClient().getChannelServer().getMapFactory().getMap(123456789));
            }
        }, 10000);
    }

    private final void startEvent(final MapleMap map) {
        map.startMapEffect("폭죽소리를 듣고 보스 몬스터들이 침공합니다!", 5120008);
    ...
    public int getSunGage() {
        if (sunshines >= 3000) {
            return 100;
        } else if (sunshines >= 2700) {
            return 90;
        } else if (sunshines >= 2100) {
            return 80;
        } else if (sunshines >= 1800) {
            return 70;
        } else if (sunshines >= 1500) {
            return 60;
        } else if (sunshines >= 1200) {
            return 50;
        } else if (sunshines >= 900) {
            return 40;
        } else if (sunshines >= 600) {
            return 30;
        } else if (sunshines >= 300) {
            return 20;
        } else if (sunshines >= 1) {
            return 10;
        } else if (sunshines == 0) {
    ...
    private final void broadcastSun(final MapleCharacter c) {
        ...
        c.ServerNoticeY("<1.CH 단풍나무언덕> 필요한 햇살이 모두모여 30초후에 단풍나무축제가 열립니다.");
    }
    ...
    private final void startSun(final MapleMap map) {
        ...
        Runnable r = new Runnable() {
            @Override
            public void run() {
                map.startMapEffect("단풍나무가 모두 성장하였습니다.", 5120008);
                spawnItem(map);
                map.resetReactors();
            }
        };
        tMan.schedule(r, 30 * 1000);
    }
    ...
    private final void spawnItem(final MapleMap map) {
        ...
        pos1 = new Point(497, 1174);
        pos2 = new Point(389, 1174);
        pos3 = new Point(598, 1174);
        pos4 = new Point(217, 1234);
        pos5 = new Point(766, 1234);

        pos6 = new Point(676, 1205);
        pos7 = new Point(305, 1205);
        pos8 = new Point(386, 1264);
        pos9 = new Point(606, 1264);
        pos10 = new Point(504, 1264);
        //4031456
        map.spawnAutoDrop(4031456, pos1);
        map.spawnAutoDrop(5060002, pos2);
        map.spawnAutoDrop(5060002, pos3);
        map.spawnAutoDrop(2049100, pos4);

        int r1 = (int) (Math.floor(Math.random() * sc.length));
        int r2 = (int) (Math.floor(Math.random() * eq.length));
        int r3 = (int) (Math.floor(Math.random() * sc.length));
        int r4 = (int) (Math.floor(Math.random() * sc.length));
        int r5 = (int) (Math.floor(Math.random() * sc.length));
        int r6 = (int) (Math.floor(Math.random() * eq.length));
        map.spawnAutoDrop(2049001, pos5);
        map.spawnAutoDrop(eq[r2], pos6);
        map.spawnAutoDrop(sc[r3], pos7);
        map.spawnAutoDrop(sc[r4], pos8);
        map.spawnAutoDrop(sc[r5], pos9);
        map.spawnAutoDrop(eq[r6], pos10);
